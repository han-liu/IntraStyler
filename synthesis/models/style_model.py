...
class StyleModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_contra', type=float, default=5.0, help='weight for style contrastive loss')
        # parser.add_argument('--lambda_kld', type=float, default=0.01, help='weight for KL divergence for VAE (beta term)')
        parser.add_argument('--lambda_temp', type=float, default=100, help='temperature value for contrastive learning')
        ...
        assert opt.netG in ['resnet_9blocks_style']
        return parser

    def __init__(self, opt):
        BaseModel.__init__(self, opt)
        self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'NCE', 'DiceA', 'DiceB', 'style', 'contra'] #'sobelL1', #, 'idt']
    ...
    def set_input(self, data):
        ...
        self.PosB = data['PosB'].to(self.device)
        self.NegB = torch.cat(data['NegB'], 0).to(self.device)
        del data
    ...
    def forward(self):
        ...
        self.real_B_style = self.netG.extract_style(self.real_B) 
        self.fake_style  = self.netG.extract_style(self.fake_B)
        self.PosB_style = self.netG.extract_style(self.PosB)
        self.NegB_style = self.netG.extract_style(self.NegB)
        # self.logits, self.label = self.netC(self.real_B_style, self.PosB_style, self.NegB_style)
        ...
        if self.opt.nce_idt:
            ...
            # self.idt_B_style  = self.netG.extract_style(self.idt_B)
    ...
    def backward_D(self):
        if self.opt.lambda_GAN > 0.0:
            ...
            # if self.epoch == XX:
            #     fake_idt = self.idt_B.detach()
            #     pred_fake_idt = self.netD(fake_idt)
            #     self.loss_D_fake_idt = self.criterionGAN(pred_fake_idt, False).mean()
            #     self.loss_D += (self.loss_D_fake_idt) * 0.5
    ...
    def backward_G(self):
        ...
        if self.opt.lambda_GAN > 0.0:
            ...
            # if self.epoch == XX:
            #     pred_fake_idt = self.netD(fake_idt)
            #     self.loss_G_GAN += self.criterionGAN(pred_fake_idt, True).mean() * self.opt.lambda_GAN            
        else:
            self.loss_G_GAN = 0.0
        if self.opt.lambda_NCE > 0.0:
        ...
        #     mu = torch.cat([self.real_B_style[1], self.fake_style[1], self.PosB_style[1], self.NegB_style[1]], 0)
        #     logvar = torch.cat([self.real_B_style[2], self.fake_style[2], self.PosB_style[2], self.NegB_style[2]], 0)
        #     self.loss_kld = self.calculate_kld_loss(mu, logvar) * self.opt.lambda_kld

        self.loss_contra = self.calculate_contrastive_loss(self.real_B_style, self.PosB_style, self.NegB_style) * self.opt.lambda_contra
        # self.loss_contra = self.calculate_contrastive_loss(self.logits, self.label) * self.opt.lambda_contra
        # self.loss_style = (self.calculate_style_loss(self.real_B_style, self.fake_style) + self.calculate_style_loss(self.real_B_style, self.idt_B_style))* self.opt.lambda_style
        self.loss_style = self.calculate_style_loss(self.real_B_style, self.fake_style) * self.opt.lambda_style

        self.loss_G = self.loss_G_GAN + loss_NCE_both + self.loss_style + self.loss_contra 
        ...
        # self.loss_G =  + self.loss_DiceA + self.loss_DiceB + self.loss_sobelL1 + #+ self.loss_idt #+ self.loss_kld 
    ...
    # def calculate_contrastive_loss(self, logits, label):
    #     return nn.CrossEntropyLoss()(logits, label)

    def calculate_contrastive_loss(self, feat, pos, neg):
        ...
        # logits = torch.matmul(feat, pairs.T) / self.opt.lambda_temp
        logits = torch.matmul(feat, pairs.T) * self.opt.lambda_temp
    ...
    def calculate_style_loss(self, real_style, fake_style):
        # sim = F.cosine_similarity(fake_style, real_style)
        sim = F.cosine_similarity(fake_style, real_style)
    ...
    def update_segB_lambda(self, epochNum):
        ...
        # if epochNum <= 200:
        #     self.opt.lambda_style = 0.0
        # else:
        #     self.opt.lambda_style = 1.0
        ...
        if epochNum <= 150:
            self.opt.lambda_style = 0.0
            self.opt.lambda_segB = 0.0
        elif epochNum > 150 and epochNum <= 300:
            self.opt.lambda_style = 1.0
            self.opt.lambda_segB = 0.5
        elif epochNum > 300 and epochNum <= 450:
            self.opt.lambda_style = 5.0
            self.opt.lambda_segB = 5.0
        else:
            self.opt.lambda_style = 10.0
            self.opt.lambda_segB = 10.0
        ...
        # print(f'lambda_style: {self.opt.lambda_style}, lambda_segB: {self.opt.lambda_segB}, temp: {self.netC.logit_scale.exp():.2f}')

        # contrast_v7
        # if epochNum <= 50:
        #     self.opt.lambda_style = 0.0
        #     self.opt.lambda_segB = 0.0
        #     self.opt.lambda_temp = 200
        # else:
        #     self.opt.lambda_style = 1.0
        #     self.opt.lambda_segB = 0.5
        #     self.opt.lambda_temp = 200 - (epochNum-50) * (180/550)
        
        # print(f'lambda_style: {self.opt.lambda_style}, lambda_segB: {self.opt.lambda_segB}, temp: {self.opt.lambda_temp:.2f}')
        ...
        #     self.opt.lambda_style = 0.0
        #     self.opt.lambda_segB = 0.0
        # elif epochNum > 300 and epochNum < 600:
        #     self.opt.lambda_style = 1.0
        #     self.opt.lambda_segB = 0.5
        #     self.opt.lambda_contra = 10.0
        # else:
        #     self.opt.lambda_style = 5.0
        #     self.opt.lambda_segB = 1.0
        #     self.opt.lambda_contra = 10.0

        print(f'lambda_style: {self.opt.lambda_style}, lambda_segB: {self.opt.lambda_segB}')