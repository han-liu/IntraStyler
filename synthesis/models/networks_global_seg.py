        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                ...
                syn_head += [
                    ...
                    norm_layer(int(ngf * mult / 2)),
                    nn.ReLU(True)]
        syn_head += [nn.ReflectionPad2d(3)]
        syn_head += [nn.Conv2d(ngf, output_nc, kernel_size=7, padding=0)]
        syn_head += [nn.Tanh()]
...
class SegHeadL(nn.Module):
    def __init__(self, ngf, n_downsampling, output_nc, norm_layer):
        ...
        for i in range(2):
            seg_head += [
                nn.Conv2d(int(ngf/(2**i)), int(ngf/(2**(i+1))), kernel_size=3, stride=1, padding=1, bias=True),
                norm_layer(ngf),
                nn.ReLU(True)]

        # output channels = 4: background, VS1, VS2, cochlea
        seg_head += [nn.Conv2d(int(ngf/4), 4, kernel_size=3, padding=1)]
        self.seg_head = nn.Sequential(*seg_head) 
...
# my_layer = torch.nn.InstanceNorm2d(num_features=24, eps=1e-05, momentum=0.1, affine=False, track_running_stats=False, device=None, dtype=None)
