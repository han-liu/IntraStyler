...
class QSSEGDYNEDGE3DModel(BaseModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_seg', type=float, default=1.0, help='weight for DiceCE loss')
        ...
        assert opt.netG in ['resnet_9blocks_dyn','resnet_9blocks']
        return parser
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'NCE', 'DiceA', 'sobelL1']
        self.visual_names = ['real_A', 'fake_B', 'real_B', 'real_A_mask', 'seg_A', 'fake_sobel', 'real_sobel', 'edge_mask']
    ...
    def forward(self):
        ...
        self.fake, self.seg = self.netG(self.real, code=self.code)
        self.fake_B = self.fake[:self.real_A.size(0)]
        self.seg_A = self.seg[:self.real_A.size(0)]

        # calculate edge
        self.fake_sobel = networks_global_seg_3D.sobelLayer(self.fake_B)
        self.real_sobel = networks_global_seg_3D.sobelLayer(self.real_A).detach()

        if self.opt.segB:
            _, self.seg_B = self.netG(self.fake_B, code=self.code)
            # _, self.seg_B = self.netG(self.fake_B)
    ...
    def backward_G(self):
        ...
        self.loss_DiceA = self.calculate_dice_loss(self.seg_A, self.real_A_mask) * self.opt.lambda_seg
        self.loss_sobelL1 = self.calculate_edge_loss(self.fake_sobel, self.real_sobel, self.edge_mask) * self.opt.lambda_sobel  # edge loss
        self.loss_G = self.loss_G_GAN + loss_NCE_both + self.loss_DiceA + self.loss_sobelL1

        if self.opt.segB:
            self.loss_DiceB = self.calculate_dice_loss(self.seg_B, self.real_A_mask)
    ...
    def calculate_dice_loss(self, src, tgt):
        # print(tgt.shape, src.shape, torch.unique(tgt))
